...
27. **Buy lists:** add an air pump next to the sponge filter (betta and zebra danio hubs in `fish.js`); add a thermostat (box turtle and red-eared slider hubs in `turtles.js`); add a TDS meter (cherry shrimp, `invertebrates.js`). [done: thermostat added to box turtle and red-eared slider buy lists (existing gear product). Air pump (betta, zebra danio) and TDS meter (cherry shrimp) have no catalog product, logged in NEEDS_PRODUCT.md]
28. **Shared small mammal guides:** in `small-mammal-vet-visits-and-travel-guide.mdx`, add ferret fasting, hedgehog warmth, rat and mouse notes, and scope "Bring the Cage Mate" to bonded species. In `small-mammal-grooming-nails-molting-guide.mdx`, add chinchilla, hedgehog, mouse and flying squirrel nails and the gerbil scent gland. In `small-mammal-temperature-heat-stress-guide.mdx`, add gerbil and mouse rows. [done except flying squirrel nails: open, no vet or university source found. Vet guide covers ferret fasting, hedgehog warmth and fast, rat and mouse recovery, cage mate scoped to bonded social species; grooming guide adds hedgehog, mouse, chinchilla nails and the gerbil scent gland; heat guide adds gerbil, mouse and degu rows]
29. **Shared reptile guides:** African fat-tail row in `reptile-emergency-plan-guide.mdx` and a zone in `uvb-lighting-complete-guide.mdx`; jumping spider row in `invertebrate-quarantine-cleaning-and-escapes-guide.mdx`; conure in the chronic egg-laying guide's scope. [done: fat-tail row in the emergency plan and Zone 1 in the UVB guide; jumping spider paragraph in the invertebrate quarantine guide; conures in the chronic egg-laying guide]
30. **Link or number where the answer lives only in the sidebar:** rabbit and hamster health guides link the enterotoxemia guide; axolotl health links the salt bath guide; box turtle and red-footed tortoise link the tortoise eggs guide; sulcata feeding carries the hatchling soak schedule; uromastyx feeding links the safe weeds guide; leopard gecko setup links the hygiene cadence; goldfish and koi setup state a pH; hub quarantine rows for discus, bristlenose, gargoyle gecko and Russian tortoise state the duration. [done: every link and number listed; bristlenose and Russian tortoise hubs gained a quarantine row]
...
37. **Move brand names out of advice prose into the buy list or a labeled recommendation:** bristlenose (Hikari, AquaClear, CaribSea), `cockatoo-tank-setup-guide.mdx:76`, `rat-cost-guide.mdx:52`, `ferret-health-issues-guide.mdx:78` (especially: it sits inside insulinoma treatment), `green-iguana-tank-setup-guide.mdx`, `red-footed-tortoise-cost-guide.mdx:62,79` (named breeder), axolotl JBJ (cost and setup), `small-mammal-vet-visits-and-travel-guide.mdx:79` (Petsfit). [done, plus the "according to" sweep across fish, invertebrate, bird, small mammal, reptile and amphibian guides; argentine-tegu-handling-guide kept as the RULES.md reference for inline naming]
38. **Source or soften the flat claims:** budgie Puck (`birds.js:84`), mantis 180 degrees (`invertebrates.js:747`), cockroach paternal care (`invertebrates.js:652`), cockatiel earthquakes, Flemish Giant water intake, guinea pig 1,181-keeper survey, hamster return rates, bearded dragon outbreak (`bearded-dragon-handling-guide.mdx:87`), veiled spay claim, pacman bite force, uromastyx 60 years and wild lifespan (`lizards.js:141`), neon "over-diagnosed", angelfish genome story (`fish.js:13`). [done: Puck credited to Guinness as an estimate; mantis "only insect" cut; cockroach paternal care replaced by sourced maternal care; cockatiel earthquake already sourced; Flemish Giant line cut; survey reworded to Elsbacher 2025; return rates softened; dragon outbreak verified against CDC; veiled spay softened and sourced; pacman bite force framed as the authors' estimate; uromastyx 60 years cut and wild lifespan corrected; neon reworded to misdiagnosis; angelfish genome paper named]
39. **Carry the caveat to where most readers see it:** the cardinal "sustainably harvested" hedge onto `fish.js:270`, the encyclopedia `fish.js:48` and the overview; degu temperature as a precaution, not a vet figure; tokay parasite stat with n=21 on the hub (`geckos.js:574`). [done: cardinal hedge on hub, encyclopedia and overview; degu 77F kept, it has a vet source (the heat guide's "couldn't verify" line was the error); tokay n=21 on the hub]
40. **Arithmetic:** green anole "twenty to thirty times" (`green-anole-cost-guide.mdx:55,111`) to a range that holds at the $5 low end. [done: at least 13 times a $30 anole and 78 to 94 times a $5 one]
