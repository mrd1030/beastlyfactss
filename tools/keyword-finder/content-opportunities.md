# Beastlyfacts.com Keyword Research — Synthesized Content Plan

Synthesized from 6 independent lenses (content-gap, legal-safety, buying-intent, comparisons, per-species-standout, curiosity-myths) over the 26,381-row keyword-ideas.csv. Overlapping findings are merged below; weak/generic signals (e.g. "X lifespan", "X cage", "X setup" topping out at 2-3 hits) are omitted.

---

## 1) Top Overall Priorities
...
3. ~~**Ferret — three distinct angles converge on one species.**~~ **DONE — all 3.**
   - ~~Naming confusion: six separate 2-hit comparisons (`ferret vs weasel`, `vs mink`, `vs stoat`, `vs mongoose`, `vs otter`, `vs polecat`) plus `is ferret a weasel`/`ferret weasel` — ~16 combined hits, zero coverage in the current FAQ.~~ **DONE** — FAQ added to ferret-handling-guide.mdx (as an FAQ addition rather than a standalone deep-dive, per the Section 3 "Single-guide comparisons" guidance).
...
## 2) Content Gaps (verified not already covered)
...
- ~~**Mourning Gecko vs House Gecko** — `mourning gecko vs house gecko` (6) + regional variants (~10 combined); current FAQ covers colony-keeping/parthenogenesis/size/diet/lifespan but never addresses the confusion. (House gecko isn't a dedicated site species, so this is a FAQ addition to the mourning gecko guide, not a standalone article.)~~ **DONE** — FAQ added to mourning-gecko-handling-guide.mdx.
...
## 3) Comparison / Internal-Linking Opportunities

The site already has 6 comparison guides (budgie-vs-cockatiel, ball-python-vs-corn-snake, bearded-dragon-vs-leopard-gecko, betta-vs-goldfish, hamster-vs-guinea-pig, leopard-gecko-vs-crested-gecko). These are excluded below — everything here is a genuine unpublished gap where **both** species already have a guide (best internal-linking ROI):
...
**Single-guide comparisons** (only one side is a tracked species — best handled as an FAQ subsection on the existing guide rather than a full new article): **Siberian Husky** (vs Alaskan Husky/Malamute/wolf, 18 hits), ~~**Ferret** (vs weasel/mink/stoat/mongoose/otter/polecat — see Priority #3)~~ **DONE** — FAQ added to ferret-handling-guide.mdx, ~~**Mourning Gecko** (vs house gecko — see Content Gaps)~~ **DONE** — FAQ added to mourning-gecko-handling-guide.mdx, **Emperor Scorpion** (vs deathstalker/Asian forest scorpion, 16 hits), **Milk Snake** (vs coral snake/king snake/copperhead, 16 hits), **Box Turtle** (vs tortoise/snapping turtle/painted turtle, 15 hits), **French Bulldog** (vs Boston Terrier/Pug/English Bulldog, 14 hits).

**Quick-win naming/definition FAQs** (near-zero effort, real search demand):
- ~~"Is a budgie the same thing as a parakeet?" (see Priority #4)~~ **DONE** — FAQ added to budgie-cost-guide.mdx.
- ~~"Boa constrictor vs python" (2 hits) — different families, commonly confused~~ **DONE** — FAQ added to boa-constrictor-handling-guide.mdx.
- ~~"Large vs giant breed dogs" (2 hits) and "what size dog is medium" (4 hits) — map directly to the site's existing Large/Medium/Small Breed Dog guides~~ **DONE** — medium-breed guide already had the "what size is medium" FAQ (verified accurate, 20-60 lbs), added the large-vs-giant FAQ to the dog-large-breed guide in src/lib/data/guides/dogs.js.

**Small variant/morph notes worth one line each:** ~~Red vs Yellow Ackie Monitor (2 hits — real color/locality variants of the same species)~~ **DONE** — FunFact added to ackie-monitor-cost-guide.mdx. ~~Pineapple vs standard Green Cheek Conure (1 hit — a color mutation, not a different bird)~~ **DONE** — FAQ added to conure-cost-guide.mdx. ~~"Tabby is a coat pattern, not a breed" clarification for Domestic Shorthair/Bengal guides (3 hits combined).~~ **DONE, already existed** — this FAQ was already on the `cat-domestic-shorthair` guide entry in src/lib/data/guides/cats.js (names Bengal explicitly), this list just hadn't been checked against it before now.

**Sexing-FAQ template** — 185 rows / 214 hits across 62 species all ask "how to tell males from females." ~~Build one reusable template, roll out in demand order: **Veiled Chameleon** (11), **Sulcata Tortoise** (7), **Pacman Frog** (7), **Box Turtle** (6), **Green Cheek Conure** (6).~~ **DONE — top 5.** Each verified via a real source before writing rather than assumed: Veiled Chameleon (tarsal spur, visible from hatching), Sulcata Tortoise (plastron/tail shape, not reliable until 10-15in shell length), Pacman Frog (nuptial pads + calling in mature males only), Box Turtle (tail/claw/eye-color, hedged since eye color isn't universal across subspecies), Conure (honestly not visually sexable at all, DNA or surgical sexing is the real answer, no plumage/head-shape tell exists despite common claims). The other 57 species in this row aren't done, this template could keep rolling if wanted.

---

## 4) Facts-Section / Curiosity Angles
...
## 5) Surprising / Unusual — Worth Flagging on Its Own

- **Fire-Bellied Toad "near me" spike (15 hits) is the single highest buying-intent suggestion in the entire dataset** — unusual for a small, cheap, obscure amphibian. Worth a closer look; may reflect a specific regional breeder/availability trend. At minimum, add a where-to-buy/sourcing section to the existing guide.
- **Niche/exotic species are punching far above their weight class.** Giant African Millipede, Ackie Monitor, Leaf-Tailed Gecko, and Argentine Tegu all show concentrated, high-value buying and safety signal despite being far more obscure than mainstream reptiles — this is the clearest actionable pattern in the whole dataset.
- **Conversely, several of the most popular pet species are "quiet" once isolated** — Bearded Dragon, Leopard Gecko, Crested Gecko, Ball Python, Boa Constrictor, and Axolotl each top out at only 2 hits on their best individual query. Likely means the site already covers their basics well; don't chase new content here based on this dataset.
- **Swiss guinea pig law** — a real, widely-cited animal-welfare rule (guinea pigs must be kept in pairs) surfaces from `is it legal to own one guinea pig in switzerland` queries. Small but fun single-fact hook, low effort.
- **Milk Snake is the only species in the dataset where "where to buy" phrasing itself reaches the 2-hit qualifying bar** — if writing "where to buy" style copy anywhere, this is the most literal match.
- **Petco-specific pricing queries** for Russian Tortoise (`russian tortoise cost petco`) and Savannah Monitor (`savannah monitor price at petco`) — possible retailer-affiliate angle if the site has or wants a Petco relationship.
- **Hermit Crab** — `how much does it cost to own a hermit crab` (2 hits) is phrased as total-cost-of-ownership, not purchase price; frame accordingly.
- **Data-quality flags — do not chase these, they're keyword-tool noise, not pet-content demand:**
  - **Persian** — top "buying" and "comparison" signal is almost entirely Persian food/culture/rug collisions (restaurants, bakeries, ice cream, rugs), not the cat breed. Real cat-buying queries exist but never clear 2 hits.
  - **Canary** → top hit is "canary islands" (travel destination, not the bird).
  - **Oscar** (fish) → top hits are "oscar isaac" (actor) and "oscar the grouch" — no usable fish signal.
  - **Goldfish** → top hit "goldfish plant" is almost certainly the houseplant, not the fish.
  - **Chinchilla** → top raw hit "chinchilla coat" (3) is likely the fur/fashion sense; use "chinchilla cage"/"chinchilla cost" instead (also 3 hits, genuinely pet-relevant).
  - **Cat/Dog generic seeds** → dominated by local-business noise (cat cafes, vet/groomer "near me") and substring collisions (Caterpillar stock price, catalytic converter cost, cataract surgery) — no real content signal.
  - **Bengal, Oscar, Koi** in the comparisons lens collide with the Cincinnati Bengals NFL team, the Academy Awards, and fintech/anime terms respectively — exclude these "vs" rows entirely from comparison-content planning.
