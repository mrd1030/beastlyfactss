...
- All times are US Eastern unless I say otherwise. Say "12:38 Eastern", not "16:38 UTC". This covers everything you tell me and everything I tell you: scheduled check-ins, deploy windows, log timestamps you quote back, "come back in an hour". Cloud sessions run on UTC, so convert before speaking, and check whether Eastern is on EDT or EST rather than assuming an offset. Where a raw UTC value matters, give Eastern first and the UTC in parentheses.
...
- Every article needs 1-2+ in-body internal links. This is enforced by scripts/check-internal-links.mjs at the front of `build`, but write the links yourself rather than relying on the check to catch it. Those links go to other species, cross-species guides, the encyclopedia profile, or an overview. Never to the same species' sibling guides or its care guide hub, and never in a sentence that exists only to carry the link: the Deep Dive list carries all of those on every page. No care-guide sentence under the opener.
