# Image prompts completed, 2026-09-17

Every prompt that produced an image already on disk, moved out of the live
IMAGE_PROMPTS.md on 2026-09-17 so that file holds the style rules and nothing
else. This is the record: when a frame needs regenerating, find its prompt here
and follow the style rules in root/IMAGE_PROMPTS.md.

Nothing here is outstanding work. Prompts are unchanged from the versions that
produced the accepted frames, including the two rewritten after a first attempt
came back wrong (the orca's dorsal fin, the satin bowerbird's avenue).
