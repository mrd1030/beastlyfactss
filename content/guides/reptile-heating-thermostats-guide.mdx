...
For an under-tank mat the answer is settled: the probe goes on the floor of the warm hide, the surface the animal actually lies on, held down with foil tape so it can't drift. For an overhead source it is not settled, and this guide previously stated it more firmly than the evidence supports. Thermostat manufacturers genuinely disagree, some specifying the probe on the basking surface directly under the lamp and others specifying it suspended in the air a few inches below the bulb, and a dimming controller behaves differently depending on which you follow. What every source agrees on is the part worth taking away: the probe reads at the animal's level, not up in the airspace near the fixture, which is the same principle VCA's snake housing guidance applies to thermometers. Beyond that, follow your controller's own instruction rather than a general rule, and confirm the result with the two independent checks below.
