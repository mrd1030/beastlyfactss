...
<ComparisonTable
  ...
  rows={[
    ...
    ["Colorado", "Legal, captive bred with documentation", "On Appendix B, so no licence is needed, but the animal must be captive bred and carry documentation that reasonably identifies it, a receipt, bill of sale or donation record"],
    ...
    ["Delaware", "Conditional", "The corn snake reaches the northern edge of its range in Delaware, so it is DNREC's animal rather than Agriculture's, and the state's carve-out is written for captive-bred aberrant colour morphs under permit"],
