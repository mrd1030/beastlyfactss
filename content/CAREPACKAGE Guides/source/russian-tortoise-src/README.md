...
`../russian-tortoise.html` is generated. Edit the fragments here, then:
    ...
    node ../_render.mjs russian-tortoise "Russian Tortoise" 2.0 --measure # overflow check
    node ../_render.mjs russian-tortoise "Russian Tortoise" 2.0           # render the PDF
...
Content cut to make pages fit goes in `../../notes/russian-tortoise-v3-notes.md`, never in the bin.

The fragments run to six page files rather than four; `PAGE_FILES` in `build.py` lists them.
The enclosure diagram on the housing page has labels anchored outside the drawing, so check
the render rather than the markup after editing it: a label that fits in the viewBox can
still be clipped at the left edge.
