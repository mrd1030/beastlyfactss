...
// LEGAL_ANIMAL_IDS below (and BEASTLYPEDIA above): a hand-copied list goes
// stale the moment a new species is added. That drift is exactly what these
// two arrays used to cause: 18 species added in one session had no
// /encyclopedia/animal/<id> or /guides/<id> route prerendered because their
// ids were never added to the old hardcoded lists here. Generated by
// scripts/generate-encyclopedia-index.js and scripts/generate-guides-index.js,
// both earlier in the same `npm run build` chain (see package.json).
const ENCYCLOPEDIA_ANIMAL_IDS = JSON.parse(
  await readFile('src/lib/generated/encyclopedia-index.json', 'utf8')
).animals.map(a => a.id);

const GUIDE_IDS = JSON.parse(
  await readFile('src/lib/generated/guides-index.json', 'utf8')
).guides.map(g => g.id);
