...
| `KY-ky-exotic-and-native.txt` | Kentucky's two rules split by nativity, the two short possession lists on the exotic side, and the rabbit written into both halves by morphology |
| `LA-la-wildlife-rules.txt` | Louisiana's six-word definition of wildlife, the January 2025 reptile rewrite with its two nonnative lists, the Green Anole Rule, and the game breeder's licence that names the flying squirrel |
| `AK-ak-5aac92-029.txt` | Alaska's clean list in full, the definition of game that keeps amphibians and invertebrates out of it, and the fetch trick for akleg.gov |
...
| `law.lis.virginia.gov` | Works with plain curl, one section per page, but a chapter index page carries no section numbers in its text; they are only in the hrefs | Scrape `chapter<N>/section<M>/` out of the chapter page, then fetch each. The department's incorporated species list is a separate 75-page PDF at dwr.virginia.gov, and an asterisk beside a name is the whole naturalized/native distinction |
| `app.leg.wa.gov` | Works with plain curl for both the RCW and the WAC | Add `&full=true` to a WAC chapter cite to get every section on one page: `/WAC/default.aspx?cite=220-450&full=true` |
| `mca.legmt.gov` | Works with plain curl, one statute section per page | `/bills/mca/title_0870/chapter_0050/part_0070/section_00NN/0870-0050-0070-00NN.html`; the part index lists the section files |
| `www.akleg.gov` | `aac.asp` and `statutes.asp` are JavaScript shells that return the same 12 KB page for every fragment and every query string, and headless Chromium fails because it does not trust the agent proxy CA | The page's own embed builder exposes a plain endpoint: `aac.asp?media=js&secStart=5.92.029&secEnd=5.92.029` and `statutes.asp?media=js&secStart=16.05.940&secEnd=16.05.940`. It returns a `document.write()` wrapper; strip the prefix, unescape the backslash-quotes, strip tags. secStart and secEnd can span a range |
| `rules.mt.gov` | Esper single-page app on S3. Every path returns the same 894-byte shell, the old `/gateway/` URLs included; `/api/policy-library` is 401 and `/api/context` is an S3 AccessDenied, and the bundle exposes no document host | No workaround found. Read the ARM from Cornell LII (`law.cornell.edu/regulations/montana/Mont-Admin-r-12.6.2205`, `.../ARM-12-6-2215`), cross-check against the MCA sections it implements, and cite the official rule number. The statute itself is fine: `mca.legmt.gov/bills/mca/title_0870/chapter_0050/part_0070/section_00NN/0870-0050-0070-00NN.html` |
| `legis.la.gov` | Works with curl, but there is no section-number URL, only `Law.aspx?d=<opaque id>`, and the ids are ordered by the section number as a STRING, so 56:8 sits near the end of Title 56 rather than the start | Bisect on the printed section number. Landmarks: 56:124 is d=104880, 56:15 is d=104900, 56:648 is d=105485, 56:8 is d=105616, 56:800 is d=105618 |
| `apps.legislature.ky.gov` | Works with curl, but a KAR page carries the regulation TWICE, the engrossed version first and the pre-engrossment "alternate view" after the regulatory impact analysis | Slice the first copy. Statutes come back as a PDF under an `.aspx` URL; get the id from `statutes/chapter.aspx?id=37729` rather than guessing, KRS 150.010 being id=55354 |
| `www.doa.la.gov` | Serves the WHOLE of LAC Title 76 as one 1.2 MB `.docx` at `/media/ln5kb5vu/76.docx` | Extract with python `zipfile` on `word/document.xml`. The file opens with a table of contents that repeats every section heading, so search for the second occurrence |
...
| `www.ksrevisor.gov` | Returns HTTP 200 with a bot-check loader page, "Please wait while your request is being verified", so a fetch looks successful and contains no statute. `ksoutdoors.gov`, `ksoutdoors.com` and `www.kdwp.state.ks.us` are Akamai-blocked on every path, which is the whole of KDWP's publishing | Statutes from `www.kslegislature.gov/li/b2025_26/statute/032_000_0000_chapter/032_<article>_0000_article/<sec>_section/<sec>_k/` with `curl -L`, since the bare URL 301s. Regulations from Cornell LII at `/regulations/kansas/K-A-R-115-20-3`; find the slug from `/regulations/kansas/agency-115/article-20` rather than guessing it |
| `www.mdwfp.com` | TLS fails with "unable to get local issuer certificate". The cert is a genuine GlobalSign OV cert; the server just omits the intermediate | Complete the chain rather than disabling verification. Fetch `http://secure.globalsign.com/cacert/gsatlasr3ovtlsca2026q1.crt` (the AIA in the leaf names it), convert with `openssl x509 -inform DER`, concatenate onto the proxy bundle and pass with `--cacert`. Then `/rules-regulations-record` lists every MDWFP rule as a PDF. The Mississippi Code itself is unreachable: justia, findlaw and public.law all refuse and casetext is retired |
| `sdlegislature.gov` statutes | `/api/Rules/<article>` was already known; the statute endpoint is `/api/Statutes/Statute/<cite>`, e.g. `40-3-24` | Note the field name differs: rules put the text in `Html` and so do statutes, but `DocumentHtml` is empty on both. `/api/Statutes/<cite>` without `/Statute/` returns the JS shell |
| `apps.sos.wv.gov/adlaw/csr/` | The Code of State Rules browser is an ASP.NET WebForms page whose only navigation is a POST back to `/`. A POST with the three hidden fields still 500s | The missing piece is `ctl00$MainContent$ddlFullTextType`, which must be `Any Of The Words` rather than empty. Send every control name, plus `ctl00$MainContent$btnSearch=Search`. Results carry `ruleview.aspx?document=<id>` per rule and that page carries `readfile.aspx?DocId=<id>&Format=WORD`. Prefer WORD: § 58-73's PDF is a scan with no text layer |
| `wvdnr.gov` | Works with plain curl, but the paths people guess (`/plants-animals/reptiles/`, `/wildlife-diversity/`) return a 404 body under HTTP 404 | The species checklists live at `/plants-animals/amphibians-reptiles/` and `/plants-animals/mammals/`, and the reptile regulation summary PDF is linked from the first of those. `/?s=<term>` searches the site but indexes articles rather than species pages |
